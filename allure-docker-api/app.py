...
GENERATE_COVERAGE_REPORT_PROCESS = '{}/generateCoverageReport.sh'.format(os.environ['ROOT'])
...
CLEAN_COVERAGE_HISTORY_PROCESS = '{}/cleanCoverageHistory.sh'.format(os.environ['ROOT'])
CLEAN_COVERAGE_RESULTS_PROCESS = '{}/cleanCoverageResults.sh'.format(os.environ['ROOT'])
...
def send_results_endpoint():  #pylint: disable=too-many-branches
    try:
        ...
        if content_type is None:
            raise Exception(
                "Header 'Content-Type' should start with 'application/json' or 'multipart/form-data'")  #pylint: disable=line-too-long
...
@app.route("/send-coverage-results", methods=['POST'], strict_slashes=False)
@app.route("/allure-docker-service/send-coverage-results", methods=['POST'], strict_slashes=False)
@jwt_required
def send_coverage_results_endpoint():  #pylint: disable=too-many-branches
    try:
        if check_admin_access(current_user) is False:
            return jsonify({'meta_data': {'message': 'Access Forbidden'}}), 403
        ...
        if (content_type.startswith('application/json') is False
                and content_type.startswith('multipart/form-data') is False):
            raise Exception("Header 'Content-Type' should start with 'application/json' or 'multipart/form-data'")  #pylint: disable=line-too-long
...
@app.route("/generate-coverage-report", strict_slashes=False)
@app.route("/allure-docker-service/generate-coverage-report", strict_slashes=False)
@jwt_required
def generate_coverage_report_endpoint():
    try:
        if check_admin_access(current_user) is False:
            return jsonify({'meta_data': {'message': 'Access Forbidden'}}), 403
        ...
        files = None
        project_path = get_project_path(project_id)
        results_project = '{}/results'.format(project_id)

        check_process(GENERATE_COVERAGE_REPORT_PROCESS, project_id)
        exec_store_results_process = '1'

        call([KEEP_HISTORY_PROCESS, project_id, ORIGIN])
        response = subprocess.Popen([
            GENERATE_COVERAGE_REPORT_PROCESS, exec_store_results_process,
            project_id, ORIGIN],
            stdout=subprocess.PIPE).communicate()[0]
...
@app.route("/clean-coverage-results", strict_slashes=False)
@app.route("/allure-docker-service/clean-coverage-results", strict_slashes=False)
@jwt_required
def clean_coverage_results_endpoint():
    try:
        if check_admin_access(current_user) is False:
            return jsonify({'meta_data': {'message': 'Access Forbidden'}}), 403
        ...
        check_process(GENERATE_COVERAGE_REPORT_PROCESS, project_id)
        check_process(CLEAN_COVERAGE_RESULTS_PROCESS, project_id)

        call([CLEAN_COVERAGE_RESULTS_PROCESS, project_id, ORIGIN])
    except Exception as ex:
        body = {
            'meta_data': {
                'message': str(ex)
